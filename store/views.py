...
from vlogs.models import Local_Factory
...
def search_view(request):
    ...
    if query:
        ...
        local_factory_results = Local_Factory.objects.filter(
            Q(name__icontains=query) | 
            Q(type__icontains=query) | 
            Q(factory_address__icontains=query) |
            Q(head_office_address__icontains=query) |
            Q(website_address__icontains=query) |
            Q(managing_director__icontains=query) |
            Q(business_contact_person__icontains=query) |
            Q(product_line__icontains=query) |
            Q(number_of_manpower__icontains=query) |
            Q(number_of_finishing_line__icontains=query) |
            Q(production_capacity__icontains=query) |
            Q(social_certification__icontains=query) |
            Q(export_country__icontains=query) |
            Q(major_buyer__icontains=query) |
            Q(production_area__icontains=query) |
            Q(other_contact_person__icontains=query) |
            Q(establishment_year__icontains=query) |
            Q(bond_license_status__icontains=query) |
            Q(investment__icontains=query)
        ).annotate(class_name=Value('Local_Factory', output_field=CharField())) 
        ...
        all_results = (
            ...
            list(categories) + list(requirements) + list(local_factory_results)
        )
